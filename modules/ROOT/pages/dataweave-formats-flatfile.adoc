...
|`substituteCharacterAsMissingValue`  |`Boolean`|`false`|If set to `true` and if the `missingValues` property is set to `none` and the `useMissCharAsDefaultForFill` is set to `true`, the flat file parser will handle the `0x1A` (SUB) character as the missing value. +
Otherwise, the flat file parser will not handle the `0x1A` character as the missing value.
