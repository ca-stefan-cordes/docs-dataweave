...
* `typeToImport`: You can use `*` to import all types defined in the schema, or to import a single type from the schema, for example, `Root`. For Avro named types like `record` and `enum` the provided name in the schema is used for the type. You can also import Avro schema types with a different name, for example, `Root as Country`. This way  you can reference the type with that name in the script.
...
The following example uses the Avro schema shown below:
...
Including the import directive from the above example in the script header loads all the existing types in the Avro schema. In `import * from avroschema!example::schema::User`, the only existing type is the `User` type, specified at the root which describes an Object that can have four properties (`name`, `email`, `address`, and `telephone`). This is equivalent to declaring the following type in your DataWeave script:
...
You can use the type to determine if a value follows the structure defined by the Avro schema.
...
The following example outputs the value `false` because the object does not contain the required field `name`:
...
DataWave generates a separate type for each named type defined in the schema. Named types include records, enums and fixed types.
