...
* `typeToImport`: Use `*` to import all types defined in the schema, or to import a single type from the schema, for example, `Root`. For Avro named types like `record` and `enum` the provided name in the schema is used for the type. You can also import Avro schema types with a different name, for example, `Root as Country`. This way you can reference the type with that name in the script.
